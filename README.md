...
- 内置午夜、石墨、森林、雪地、薄雾和沙地 6 套主题，并会记住选择。
...
- 收藏夹支持新建、重命名和默认收藏夹；全屏中可一键收藏当前媒体。
...
- 点击图片或视频进入全屏查看，支持最近播放记录、顺序/随机播放，以及双栏/四栏多媒体观看。
- 全屏左侧显示标签按钮，可为当前媒体添加或取消多个标签；标签状态和列表页同步。
- 全屏中点击作者搜索会保存当前观看上下文，搜索结果列表顶部可一键“返回视频”，回到原来的集合、排序和播放项。
...
- 图片和视频支持手动画面旋转；可锁定当前旋转角度，让全屏、多媒体窗格和浮窗预览继续沿用同一默认角度。
- 视频支持进度条拖动、当前时间/总时长显示、`0.7x`、`0.9x`、`1.2x`、`1.5x` 固定倍速播放，以及片段标记和命名。
- 全屏提供色调曲线、亮度/对比度等颜色调整和可保存的预设；视频会在调整后重建播放管线并回到原进度，减少色彩调整不生效或播放状态丢失的问题。
...
- 设置页新增 `ts视频转换`，可扫描应用显示的固定 `share91` 导入目录，将本地 `.m3u8`、`.ts` 分片和 `tsKey` 组合转换为 MP4，并下载发布到系统媒体库。
- TS 转换会跳过已下载记录、忽略远程清单，报告缺失 key 或分片等扫描诊断，并支持单个转换/下载或全部转换并下载。
...
设置页的 `ts视频转换` 用于处理已经在手机本地的 HLS 分片目录。应用会显示固定导入目录，默认使用应用私有外部目录下的 `share91` 文件夹。把每个视频的本地 `.m3u8`、`.ts` 分片和可选 `tsKey` 放到同名目录后，点击“扫描”即可发现待转换视频。

- 远程 `.m3u8` 会被当作外层清单忽略；如果同目录下存在同名子目录和本地清单，会优先解析本地清单。
- AES-128 key 需要是本地文件且为 16 字节；缺失 key、缺失分片或没有本地分片时会显示扫描诊断。
- “转换”会先在应用缓存中生成 MP4；“下载”会把 MP4 发布到系统媒体库，默认目录为 `Movies/VideoSee`。
- “全部转换并下载”会按列表批量处理，并在顶部提示成功/失败统计。
- 仓库根目录的 `Movies/` 只用于本地样本或临时导入，已在 `.gitignore` 中忽略，不应提交个人媒体或 HLS 分片。
...
- 导出的 JSON 包含作者爱心、媒体爱心、标签名称、媒体标签映射、收藏夹和视频片段，建议定期覆盖保存一份最新备份。
...
- 截图和下载文件会写入系统媒体库目录。
- TS 转换会读取应用显示的本地导入目录，并在转换完成后把 MP4 写入系统媒体库。
- 下载 Token 存储在应用本地偏好设置中；不要使用公开或复用的敏感 Token。
- 仓库不应提交真实 Token、个人媒体文件、HLS 分片、Debug APK、签名密钥、`local.properties`、Gradle 缓存、构建产物或临时图标源文件。

...
- 左侧支持文件夹浏览和作者浏览，右侧显示当前分类内的图片和视频缩略图。
- 作者浏览会识别 `{author_id}_{timestamp}_{media_id}` 这类至少包含两个下划线的文件名，并按 `author_id` 自动归类；不符合规则的文件不会进入作者归类。
- 主页面提供手动刷新按钮，用于重新扫描新增图片和视频。
- 左侧分类支持按名称、数量、最新修改时间排序，并支持正序/倒序。
- 右侧媒体列表支持按名称、最新修改时间排序，并支持正序/倒序。
- 点击图片或视频后进入全屏查看。
- 上滑/下滑切换上一个或下一个媒体。
- 视频双击播放/暂停。
